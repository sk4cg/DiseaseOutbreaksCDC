...
Before building our models, we needed to do some additional preprocessing to our data in order to get it ready for building informative models. We decided that we would focus on 3 different predictor variables: `Month`, `Food`, and `State`. And hopefully these features would considerably aid us in training models to predict our target variable, `Species`.  At this stage in our analysis the dataset, provided us with `Month`  as a factor variable with 12 levels, `State` as a factor variable with 55 levels (it included DC and US territories in addition to the 50 states), `Food` as a factor variable with 3128 levels, and `Species` as a factor variable with 202 levels. We believe that the datapoints for Food and Species contained hand entered variable (rather than choosing from a pre-existing list) which opened it up to a lot of variability in responses and would make it harder for the models to classify with so many levels. Additionally, in order to create the decision trees there was a limit of 32 levels and the as is data did not fit those requirements. Thus, we needed to filter down each variable that had a high amount of levels and filter it into more manageable distinctions. While we do recognize that this added bias to our models and especially disadvantaged the variables that were less common in our dataset (as they were put in the "Other" category), we were hoping it would still provide us with meaningful models that were able to classify Species. 

I chose not to filter down the `Month` variable as I felt it was not necessary and that it would be more informative to keep it as is than to make up a reasoning on how to group them. To filter the variable I used a function that checked each data point in a given feature for different regex expressions that I specified. For instance for the Food variable I checked for data points that contained "fish", "salmon", or "tuna" and if they contained any of those regex expressions they would be classified into the "Fish" level, and so on. For Food I chose overarching categories of different types of food such as chicken, vegetarian, pork, beef etc. For the Species variable I filtered the levels by choosing the most prevalent species in the dataset and choosng those as levels, and the rest were classified into the Other category. And for the States, I used the Koppen climate classification which for the US and its territories, encompasses 4 different climate levels: Tropical, Dry (Arid and Semiarid), Temperate (Mesothermal), and Continental (Microthermal). Some states do have high variation in climate regions, and for those states I chose the level type that encompassed the largest percentage (majority) of land in the state. I also included an "other" level to catch any mis-entered data. 

After preprocessing and filtering the data, the `Food` variable had 8 levels, the `Species` variable had 10 levels, the `State` variable had 5 levels, and the `Month` variable which was not altered had 12 levels. 
...
First, I needed to do some additional preprocessing before executing KNN. The R KNN function requires numerical data, and since our feature variables were all categorical at this point, I chose to map each factor level to a number and then run KNN. It was not necessary to map the Species variable as it was the target variable. 
```{r KNNpreprocess}
...
Next, I used the sample function to split the data 3:1 into 75% training and 15% testing subsets, separated the X-variables and the y-labels and set a constant seed value in addition to scaling and centering the x-variables. Scaling and centering the x-variables allows KNN to calculate distances in a more meaningful manner 
...
Next, I ran knn and initially looped through the k-values from 3 to 21, but here we will show the confusion matrix from 3 to 7 for a better visual representation and looked at the accuracy, precision, recall, F1 and other metrics using the confusion matrix to determine the best k value. 
...
```
I decided the best k value for KNN was k=5 which gave us an overall accuracy of 38.5%. This was due to the fact that as k reached k=5, the accuracy had higher jumps in improvement. For example between k=3 and k=4, there was a 5% jump in accuracy and between k=4 and k=5 there was a 2.3% jump in accuracy. And after k=5, with each increase in k, accuracy only increased by around 0.6% each time, there wasnt much improvement. Eventually if I chose a very large k value that would bring the accuracy up to around 43% or higher but that would be at the expense of precision. 

If we look at the k=5 value in the confusion matrix, there are a few things I noticed that we would like to highlight. For balanced accuracy the Ciguatoxin came at 72.7% and Scombroid toxin has 63.8% and the rest of the features had roughly 50% balanced accuracy. And for the F1 scores, Ciguatoxin had 41.5% and Norovirus has 43.9% and Scombroid toxin has 32.9%. Looking at these values especially, we were able to conclude that KNN was able to classify Ciguatoxin and Scombroid toxin the most reliably and had trouble with the other causative agents but did have some success. 
...
There were a variety of factors associated with the low accuracy of the models. First and foremost, there were simply too many levels to each variable. Attempting to group the Food, States, and Species was a difficult task. Each pathogen is unique and grouping them into only a few categories greatly increased error. The same occurred with the states. The states were grouped by climate, which is associated with the types of food outbreaks that occur. However, climate AND location (west coast versus east coast, etc.) are important when indicating whether it is likely to be one causative agent or another. Grouping the food by category was very difficult. There were thousands of variations of foods, as they were specific to the individual. Though the food is very important in determining causative agent (i.e. Salmonella grows on chicken), the environment of the food is also important. For instance, though there isn't anything specific in potato salad that would clue you in to food poisoning, the fact that potato salad is often eaten at picnics where people leave food out for hours is a major variable. Additionally, Staphylococcus aureus and Shigella are both organisms that come from preparing food with dirty hands. So, food that is often prepared by hand, and not just the fact that it may be fish or chicken, is imporant. Because of these reasons, the food grouping made it very difficult to group the data. Increasing the number of groups to the maximum number of levels allows (32) or to use a different method of grouping may have increased the accuracy. 
