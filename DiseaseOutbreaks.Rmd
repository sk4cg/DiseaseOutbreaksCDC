...
Before building our models, we needed to do some additional preprocessing to our data in order to get it ready for building informative models. We decided that we would focus on 3 different predictor variables: `Month`, `Food`, and `State`. And hopefully these features would considerably aid us in training models to predict our target variable, `Species`.  At this stage in our analysis the dataset, provided us with `Month` as a factor variable with 12 levels, `State` as a factor variable with 55 levels (it included DC and US territories in addition to the 50 states), `Food` as a ctorfa variable with 3128 levels, and `Species` as a factor variable with 202 levels. We believe that the datapoints for Food and Species contained hand entered variable (rather than choosing from a pre-existing list) which opened it up to a lot of variability in responses and would make it harder for the models to classify with so many levels. Additionally, in order to create the decision trees there was a limit of 32 levels and the as is data did not fit those requirements. Thus, we needed to filter down each variable that had a high amount of levels and filter it into more manageable distinctions. While we do recognize that this added bias to our modells and especially disadvantaged the variables that were less common in our dataset (as they were put in the "Other" category), we were hoping it would still provide us with meaningful models that were able to classify Species. 

I chose not to filter down the `Month` variable as I felt it was not necessary and that it would be more informative to keep it as is than to make up a reasoning on how to group them. To filter the variable I used a function that checked each data point in a given feature for different regex expressions that I specified. For instance for the Food variable I checked for data points that contained "fish", "salmon", or "tuna" and if they contained any of those regex expressions they would be classified into the "Fish" level, and so on. For Food I chose overarchign different types of food such as chicken, vegetarian, prok, beef etc. For the Species variable I filtered the levels by choosing the most prevalent species in the dataset and choosign those as levels, and the rest were classified into the Other category. And for the States, I used the Koppen climate classification which for the US and its territories, encompasses 4 different climate levels: Tropical, Arid, Temperate, and Polar  
